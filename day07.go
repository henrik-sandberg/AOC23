...
func Day07(input []string) {
	...
	fmt.Println("Result part 1:", sumCardBid(input))
	sort.Slice(input, func(i, j int) bool {
		return isLess(
			strings.Fields(input[i])[0],
			strings.Fields(input[j])[0],
	...
	fmt.Println("Result part 2:", sumCardBid(input))
}
...
func isLess(card1, card2 string, maxOccurrences func(string) []int, cardValue []rune) bool {
	...
	for i := 0; i < len(card1); i++ {
		ai := IndexOf(cardValue, rune(card1[i]))
		bi := IndexOf(cardValue, rune(card2[i]))
		if ai != bi {
			return ai < bi
		}
	}
